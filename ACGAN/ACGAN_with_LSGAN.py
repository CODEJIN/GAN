...
class ACGAN:
    def __init__(self, noise_Size = 100, hidden_Unit_Size = 128, learning_Rate = 0.0002):
    ...
    def Tensor_Generate(self):        
        ...
        self.label_for_Fake_Placeholder = tf.placeholder(tf.float32, shape=(None, 10), name="label_Placeholder");
        self.label_for_Original_Placeholder = tf.placeholder(tf.float32, shape=(None, 10), name="label_Placeholder");
        ...
        generator_Activation = Generator_with_Vanilla(
            noise_Pattern = self.noise_Placeholder, 
            label_Pattern = self.label_for_Fake_Placeholder, 
            hidden_Unit_Size = self.hidden_Unit_Size, 
            output_Unit_Size = 28*28, 
            is_Training = self.
        ...
        discrimination_Logit_for_Fake, label_Logit_for_Fake, discrimination_Activation_for_Fake, label_Activation_for_Fake = \
            Discriminator_with_Vanilla(                
                input_Pattern = generator_Activation, 
                hidden_Unit_Size = self.hidden_Unit_Size,
                label_Unit_Size = 10,
        ...
        discrimination_Logit_for_Original, label_Logit_for_Original, discrimination_Activation_for_Original, label_Activation_for_Original = \
            Discriminator_with_Vanilla(                
                input_Pattern = self.original_Placeholder, 
                hidden_Unit_Size = self.hidden_Unit_Size,
                label_Unit_Size = 10,
                is_Training  = self.is_Training_Placeholder,
                reuse = True
        ...
        discriminator_Loss_for_Fake = tf.reduce_mean((discrimination_Target_Pattern_for_Fake - discrimination_Logit_for_Fake) ** 2);
        discriminator_Loss_for_Original = tf.reduce_mean((discrimination_Target_Pattern_for_Original - discrimination_Activation_for_Original) ** 2);
        ...
        with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
            generator_Optimize = tf.train.AdamOptimizer(self.learning_Rate, beta1=0.9, beta2=0.999).minimize(
                generator_Loss, 
                var_list= generator_Variable_List,
                global_step = global_Step
                )
        
            discriminator_Optimize = tf.train.AdamOptimizer(self.learning_Rate, beta1=0.9, beta2=0.999).minimize(
                discriminator_Loss, 
...
if __name__ == "__main__":
    new_ACGAN = ACGAN(noise_Size = 100, hidden_Unit_Size = 256, learning_Rate = 0.0002);
    new_ACGAN.Train(test_Step = 1000);
